...
function createLaunchctlServiceController(identifier, options = {}) {
  const domain = options.domain || "system";
  const plistPath =
    options.plistPath ||
    (domain === "system"
      ? path.join("/Library", "LaunchDaemons", `${identifier}.plist`)
      : path.join(
          os.homedir(),
          "Library",
          "LaunchAgents",
  ...
  const scopedIdentifier = `${domain}/${identifier}`;
  ...
  return {
    ...
    async status() {
      ...
      if (result.exitCode === 0) {
        const running =
          /\bpid\s*=\s*\d+/i.test(output) ||
          /\bstate\s*=\s*running\b/i.test(output);
        return {
          running,
          state: running ? "running" : "stopped",
          source: "launchctl",
          detail: running
            ? `launchctl reports ${domain} service is running`
            : "launchctl loaded but no PID present",
        };
      }

      const fallback = await execCommand("launchctl", ["list", identifier]);
      ...
      if (fallback.exitCode === 0) {
        const running = /"PID"\s*=\s*\d+/i.test(fallback.stdout || "");
        return {
          running,
          state: running ? "running" : "stopped",
          source: "launchctl",
          detail: running
            ? `launchctl fallback reports ${domain} service is running`
            : "launchctl loaded but no PID present",
        };
      }
      ...
      if (
        result.exitCode === 113 ||
        /label not found|could not find service|service not found/i.test(
      ...
      return {
        ...
        state: "unknown",
        source: "launchctl",
        detail: fallbackOutput || output || "launchctl unavailable",
      };
    },
    async start() {
      await execPosixShellWithElevationFallback(
        [
          `launchctl enable ${scopedIdentifier} >/dev/null 2>&1 || true`,
          `launchctl bootstrap ${domain} ${quotePosixShellArg(plistPath)} >/dev/null 2>&1 || true`,
          `launchctl kickstart -k ${scopedIdentifier}`,
        ].join(" && "),
        { alwaysElevateOnFailure: true },
      );
    },
    async enable() {
      await execPosixShellWithElevationFallback(
        `launchctl enable ${scopedIdentifier}`,
        { alwaysElevateOnFailure: true },
      );
    },
    async stop() {
      await execPosixShellWithElevationFallback(
        [
          `launchctl disable ${scopedIdentifier} >/dev/null 2>&1 || true`,
          `launchctl bootout ${scopedIdentifier} >/dev/null 2>&1 || true`,
          `launchctl bootout ${domain} ${quotePosixShellArg(plistPath)} >/dev/null 2>&1 || true`,
        ].join(" ; "),
        { alwaysElevateOnFailure: true },
      );
    },
    async disable() {
      await execPosixShellWithElevationFallback(
        `launchctl disable ${scopedIdentifier}`,
        { alwaysElevateOnFailure: true },
      );
    },
  };
...
function shouldUseConfiguredPrivilegeFallback(config = null) {
  const resolvedConfig = config || readClientConfig();
  return Boolean(resolvedConfig && resolvedConfig.runAsRoot === true);
}
...
function shouldAttemptPosixPrivilegeFallback(errOrResult) {
  const text = String(
    errOrResult?.message || collectResultText(errOrResult) || errOrResult || "",
  ).toLowerCase();
  return [
    "eacces",
    "eperm",
...
async function execPosixShellWithElevationFallback(shellCommand, options = {}) {
  const {
    alwaysElevateOnFailure = false,
    requirePrivilegePreference = false,
    config = null,
  } = options;

  const direct = await execCommand("/bin/sh", ["-lc", shellCommand]);
  if (direct.exitCode === 0) {
    return direct;
  }
  ...
  if (
    requirePrivilegePreference &&
    !shouldUseConfiguredPrivilegeFallback(config)
  ) {
    throw new Error(directMessage);
  }

  if (!alwaysElevateOnFailure && !shouldAttemptPosixPrivilegeFallback(direct)) {
    throw new Error(directMessage);
  }
  ...
  for (const candidate of candidates) {
    const result = await execCommand(candidate.command, candidate.args);
    if (result.exitCode === 0) {
      return result;
    }
...
async function writeClientConfigFile(targetPath, config, options = {}) {
  const { preferPrivilegeFallback = false } = options;
  try {
...
`;
    ...
    if (
      ...
      preferPrivilegeFallback &&
      shouldAttemptPosixPrivilegeFallback(err)
    ) {
      const payload = JSON.stringify(config, null, 2);
      const tempPath = path.join(
        os.tmpdir(),
        `breakeven-dashboard-config-${process.pid}-${Date.now()}.json`,
      );
      const shellCommand = [
        `mkdir -p ${quotePosixShellArg(path.dirname(targetPath))}`,
        `cp ${quotePosixShellArg(tempPath)} ${quotePosixShellArg(targetPath)}`,
        `chmod 0644 ${quotePosixShellArg(targetPath)}`,
      ].join(" && ");

      try {
        fs.writeFileSync(tempPath, payload, "utf8");
        await execPosixShellWithElevationFallback(shellCommand, {
          alwaysElevateOnFailure: true,
          requirePrivilegePreference: true,
          config,
        });
        return { ok: true, path: targetPath, elevated: true };
      } catch (elevatedErr) {
...
async function writeClientConfigToTargets(config) {
  ...
  const configTargets = dedupePaths([
    ...
    effectiveServicePath &&
      path.join(effectiveServicePath, "client_service", "client_config.json"),
  ]);
  const primaryTarget =
    getClientConfigCandidates(configTargets).find((candidate) => {
      try {
  ...
  const secondaryTargets = configTargets.filter(
    (candidate) => path.resolve(candidate) !== path.resolve(primaryTarget),
  );

  const preferPrivilegeFallback = shouldUseConfiguredPrivilegeFallback(config);
  const results = [
    await writeClientConfigFile(primaryTarget, config, {
      preferPrivilegeFallback,
    }),
  ];
  if (results[0]?.ok) {
  ...
  for (const target of secondaryTargets) {
    results.push(
      await writeClientConfigFile(target, config, {
        preferPrivilegeFallback,
      }),
    );
  }
...
function resolveWorkRecordsDirPath() {
  const serviceDir = resolveClientServiceDir();
  if (!serviceDir) return null;
  return path.join(serviceDir, "logs", "work_records");
}

function summarizeWorkRecord(record, filePath, fileStats) {
  const work = record?.work || {};
  const slave = record?.slave || {};
  ...
  return {
    ...
    filePath,
    updatedAt: record?.updated_at || fileStats?.mtime?.toISOString?.() || null,
    createdAt:
      record?.created_at || fileStats?.birthtime?.toISOString?.() || null,
    workId: work.work_id || null,
    workName: work.work_name || null,
    workType: work.work_type || null,
    traceId: work.trace_id || null,
    workStatus: work.status || null,
    workSuccess:
      typeof work.work_success === "boolean" ? work.work_success : null,
    slaveId: slave.slave_id ?? null,
    slaveName: slave.slave_name || null,
    slaveVersion: slave.version || null,
    slaveHostname: slave.hostname || null,
    slaveMachineUid: slave.machine_uid || null,
    slavePid: slave.pid ?? null,
    masterStatus: result.master_status || null,
    masterAckStatus: result.master_ack_status || null,
    acceptedByMaster: result.master_status === "accepted",
    rejectedByMaster: result.master_status === "rejected",
    workersUsed: execution.workers_used ?? null,
    peakCoresUsed: execution.peak_cores_used ?? null,
    availableCoresAtStart: execution.available_cores_at_start ?? null,
    totalLogicalCores: execution.total_logical_cores ?? null,
    bufferCores: execution.buffer_cores ?? null,
    parallelBackend: execution.parallel_backend || null,
    sharesAssigned: allocation.shares_assigned ?? null,
    totalShares: allocation.total_shares ?? null,
    sharePercent: allocation.share_percent ?? null,
    sharesProcessed: allocation.shares_processed ?? null,
    payloadBytes: allocation.payload_bytes ?? null,
    payloadMb: allocation.payload_mb ?? null,
    receivedAt: timing.work_received || null,
    payloadFetchStartedAt: timing.payload_fetch_start || null,
    payloadFetchCompletedAt: timing.payload_fetch_end || null,
    payloadFetchMs: timing.payload_fetch_ms ?? null,
    startedAt: timing.work_started || null,
    completedAt:
      timing.processing_completed ||
      timing.result_last_sent ||
      timing.result_prepared ||
      timing.ack_received ||
      null,
    resultPreparedAt: timing.result_prepared || null,
    resultFirstSentAt: timing.result_first_sent || null,
    resultLastSentAt: timing.result_last_sent || null,
    ackReceivedAt: timing.ack_received || null,
    processingMs: timing.processing_ms ?? null,
    turnaroundMs: timing.total_slave_turnaround_ms ?? null,
    queueDelayMs: timing.queue_delay_ms ?? null,
    processingToResultSendMs: timing.processing_to_result_send_ms ?? null,
    ackLatencyMs: timing.ack_latency_ms ?? null,
    resultSent: result.result_sent ?? null,
    sendAttempts: result.send_attempts ?? null,
    lastSendAttempt: result.last_send_attempt ?? null,
    resultAckReceived: result.result_ack_received ?? null,
    masterAckMessage: result.master_ack_message || null,
    payoutWallet: payout.wallet || null,
    terminatedByMaster: termination.terminated_by_master === true,
    terminationReason: termination.termination_reason || null,
    terminationTime: termination.termination_time || null,
    terminationProgressPercent:
      termination.termination_progress_percent ?? null,
    errorCount: errors.length,
    errors,
  };
}

async function readRecentWorkRecords(limit = null) {
  const workRecordsDir = resolveWorkRecordsDirPath();
  if (!workRecordsDir || !fs.existsSync(workRecordsDir)) {
    return [];
  }

  const entries = await fs.promises.readdir(workRecordsDir, {
    withFileTypes: true,
  });
  const jsonFiles = entries
    ...
    .map((entry) => path.join(workRecordsDir, entry.name));
  ...
  recordsWithStats.sort((left, right) => {
    const rightTime = right.stats.mtimeMs || 0;
    const leftTime = left.stats.mtimeMs || 0;
    if (rightTime !== leftTime) {
      return rightTime - leftTime;
    }
    return path
      .basename(right.filePath)
      .localeCompare(path.basename(left.filePath));
  });

  const normalizedLimit = Number.isFinite(limit)
    ? Math.max(Math.trunc(limit), 1)
    : null;
  const selectedRecords = normalizedLimit
    ? recordsWithStats.slice(0, normalizedLimit)
    : recordsWithStats;
  const summaries = await Promise.all(
    selectedRecords.map(async ({ filePath, stats }) => {
      try {
        const raw = await fs.promises.readFile(filePath, "utf-8");
        const parsed = JSON.parse(raw);
        return summarizeWorkRecord(parsed, filePath, stats);
      } catch (error) {
        return {
          fileName: path.basename(filePath),
          filePath,
          ...
          slaveVersion: null,
          slaveHostname: null,
          slaveMachineUid: null,
          slavePid: null,
          masterStatus: null,
          masterAckStatus: null,
          acceptedByMaster: false,
          rejectedByMaster: false,
          workersUsed: null,
          peakCoresUsed: null,
          availableCoresAtStart: null,
          totalLogicalCores: null,
          bufferCores: null,
          parallelBackend: null,
          sharesAssigned: null,
          totalShares: null,
          sharePercent: null,
          ...
          payloadMb: null,
          receivedAt: null,
          payloadFetchStartedAt: null,
          payloadFetchCompletedAt: null,
          payloadFetchMs: null,
          startedAt: null,
          completedAt: null,
          resultPreparedAt: null,
          resultFirstSentAt: null,
          resultLastSentAt: null,
          ackReceivedAt: null,
          processingMs: null,
          turnaroundMs: null,
          queueDelayMs: null,
          processingToResultSendMs: null,
          ackLatencyMs: null,
          resultSent: null,
          sendAttempts: null,
          lastSendAttempt: null,
          resultAckReceived: null,
          masterAckMessage: null,
          payoutWallet: null,
          terminatedByMaster: false,
          terminationReason: null,
          terminationTime: null,
          terminationProgressPercent: null,
          errorCount: 1,
          errors: [`Failed to read record: ${error.message}`],
        };
      }
    }),
  );
...
function startSlaveLogTail(win) {
  ...
  function parseSlaveLogBuffer(chunkText, options = {}) {
    const { carryover = "", discardLeadingFragment = false } = options;
    let combined = `${carryover}${chunkText || ""}`;
    ...
    if (
      discardLeadingFragment &&
      combined[0] !== "\r" &&
      combined[0] !== "\n"
    ) {
      const firstBreakIndex = combined.search(/\r|\n/);
      if (firstBreakIndex === -1) {
        return { lines: [], carryover: "" };
      }

      combined =
        combined[firstBreakIndex] === "\r" &&
        combined[firstBreakIndex + 1] === "\n"
          ? combined.slice(firstBreakIndex + 2)
          : combined.slice(firstBreakIndex + 1);
    }
    ...
    for (let index = 0; index < combined.length; index += 1) {
      ...
      if (character === "\r" || character === "\n") {
        const normalizedLine = current.trimEnd();
  ...
  const readNewData = () => {
    fs.stat(logPath, (err, stats) => {
      ...
      stream.on("end", () => {
        const parsed = parseSlaveLogBuffer(buffer, {
          carryover: trailingLineFragment,
          discardLeadingFragment: firstRead && startPos > 0,
        });

        trailingLineFragment = parsed.carryover;
        lastSize = stats.size;
        firstRead = false;

        if (parsed.lines.length > 0 && !win.isDestroyed()) {
          win.webContents.send("slave-log-lines", parsed.lines);
        }
      });
    });
  };
...
ipcMain.handle("get-slave-log", async () => {
  try {
    ...
    const trailingLine = current.trimEnd();
    if (trailingLine.length > 0) {
      lines.push(trailingLine);
    }

    return lines.slice(-10000);
  } catch (err) {
...
ipcMain.handle("get-work-records", async (_event, options = {}) => {
  try {
    const requestedLimit = Number(options?.limit);
    const limit = Number.isFinite(requestedLimit)
      ? Math.max(Math.trunc(requestedLimit), 1)
      : null;
    return await readRecentWorkRecords(limit);
  } catch (err) {
    console.error("[Electron] get-work-records error:", err);
    return [];
  }
});
...
function resolveSlaveLaunchCommand(serviceDir) {
  ...
  if (process.platform === "linux") {
    const runnerPath = path.join(
      serviceDir,
      "breakeven_slave_service_runner.sh",
    );
    if (fs.existsSync(runnerPath)) {
      return { command: "/bin/bash", args: [runnerPath], shell: false };
    }
  }
...
function resolveTrayLaunchCommand(serviceDir) {
  ...
  if (process.platform === "linux") {
    const runnerPath = path.join(
      serviceDir,
      "breakeven_tray_service_runner.sh",
    );
    if (fs.existsSync(runnerPath)) {
      return { command: "/bin/bash", args: [runnerPath], shell: false };
    }
...
async function setTrayStartupEnabled(enabled) {
  ...
  if (!launch) {
    throw controllerError || new Error("No tray startup command available");
  }
